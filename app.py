...
def keyword_process(keyword):
    ...
    if keyword == "_4차산업":
        query = "select 관리기관, 지원분야, 공고명, 신청시작일자, 신청종료일자, 공고상세URL, 등록일자 FROM rcmd_dictionary_cluster0;"
    elif keyword == "_친환경":
        query = "select 관리기관, 지원분야, 공고명, 신청시작일자, 신청종료일자, 공고상세URL, 등록일자 FROM rcmd_dictionary_cluster1;"
    elif keyword == "_bio_health":
        query = "select 관리기관, 지원분야, 공고명, 신청시작일자, 신청종료일자, 공고상세URL, 등록일자 FROM rcmd_dictionary_cluster2;"
    elif keyword == "_기술_컨설팅":
        query = "select 관리기관, 지원분야, 공고명, 신청시작일자, 신청종료일자, 공고상세URL, 등록일자 FROM ict1.rcmd_kmeans_cluster0;"
    elif keyword == "_융자_자금지원":
        query = "select 관리기관, 지원분야, 공고명, 신청시작일자, 신청종료일자, 공고상세URL, 등록일자 FROM ict1.rcmd_kmeans_cluster1;"
    elif keyword == "_창업_일자리":
        query = "select 관리기관, 지원분야, 공고명, 신청시작일자, 신청종료일자, 공고상세URL, 등록일자 FROM ict1.rcmd_kmeans_cluster2;"
    elif keyword == "_해외수출_박람회":
        query = "select 관리기관, 지원분야, 공고명, 신청시작일자, 신청종료일자, 공고상세URL, 등록일자 FROM ict1.rcmd_kmeans_cluster3;"       
...
def query_main(keyword, keyword_list):
    if len(keyword_list) == 0:
        ...
        query_main = f"select 관리기관, 지원분야, 공고명, 신청시작일자, 신청종료일자, 공고상세URL, 등록일자, {score_query} FROM mlv2_20220929 \
                    WHERE {cond_query}  \
                    ORDER BY case when 공고명 = '{keyword}' then 1 else 2 end, 등록일자 Desc, Score Desc;"
    return query_main
